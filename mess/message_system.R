...
# new notify function that uses rlang::info

notify <- function(... , detail = 1, warning = FALSE, frame = 1) {
  if (detail > 1) return()
  msg <- paste(c(...), collapse = "") |> glue::glue()
  msg <- setNames(msg, rep("!", length(msg)))
  if (!warning) {
    rlang::inform(msg)
  } else {
    rlang::warn(msg)
