...
A dictionary file is a table with a row for each variable and a column for each atttribute of those variables. Most conveniently, you prepare a dictionary file in a spreadsheet program for later use with data sets.
...
A dictionary file can contain any additional attributes you like. That is, you can add a column with any name to store relevant information (e.g. scale and scale label an item belongs to, a translation of the item name). But some predefined attributes exist with a specific meaning. The following table shows these attributes:
