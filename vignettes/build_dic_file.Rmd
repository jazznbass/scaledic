...
We get the message that the dic file does not contain information of the data*type* (e.g. `numeric`, `factor`, `character`). The `type` attribute is used in various *scaledic* functions. For example when the data is checked for invalid values, missing values are imputed, or scales are scored. A variable can be one of the following types: `integer` for numbers without decimals, `numeric` for numbers with or without decimals, `character` for variables with text, and `factor` for variables with text or numbers that are levels of a factor. 
...
```{r echo=FALSE}
dic_file <- ex_scaledic_dic |> subset(select = c(1,2,type))
...
```{r echo=FALSE}
dic_file <- ex_scaledic_dic |> subset(select = c(1,2, weight, type))
...
```{r echo=FALSE}
dic_file <- ex_scaledic_dic |> subset(select = c(1,2, values, weight, type))
...
```{r echo=FALSE}
dic_file <- ex_scaledic_dic |>subset(select = c(1,2, values, value_labels, weight, type))
...
```{r echo=FALSE}
dic_file <- ex_scaledic_dic |>subset(select = c(1,2, values, value_labels, weight, type, missing))
