...
dic_file |> wmisc::nice_table(title = "Dictionary file")
```
...
dic_file |> wmisc::nice_table(caption = "Dictionary file")
```
...
dat_dic %>%
  slice(9:20) |>
  wmisc::nice_table(title = "Extract from the data frame with replaced missing values and checked values")
```
